...
Let an interviewer create and manage an interview session.

Generate a secure, shareable candidate link.

Allow multiple participants to join from modern desktop browsers.

Provide a low-latency, real-time shared canvas.

Support structured system-design diagrams and freehand drawing.

Recover gracefully from brief network interruptions.

Save the interview canvas automatically for later review.

Clearly identify participants and show who is currently active.
...
The interviewer signs in and selects New interview.

The interviewer enters a title and optional problem statement.

The system creates a draft session and a blank canvas.

The interviewer optionally pre-populates the canvas or selects a template.

The interviewer selects Create candidate link.

The system generates a high-entropy, revocable link.

The interviewer copies and sends the link to the candidate.

5.2 Join an interview

The candidate opens the link.

The system validates that the token is active and the session is joinable.

The candidate enters a display name and accepts a short privacy notice.

The candidate joins the canvas.

Existing participants see the candidate appear in the participant list.

The candidate receives the current canvas state, then live updates.

5.3 Conduct the interview

Participants place components, draw, add text, and connect elements.

Each participant sees remote cursors, selections, and edits in real time.

Changes save automatically.

If a participant disconnects, local edits are queued and synchronized after reconnection when safe.

The interviewer may temporarily lock candidate editing without removing the candidate.

5.4 End and review

The owner selects End interview and confirms.

The session becomes read-only for candidates.

The system saves a final snapshot.

The interviewer can reopen the result from the interview dashboard.

The interviewer can export the canvas as PNG or PDF in a post-MVP increment; JSON export is recommended in the MVP for support and portability.
...
Web/API service: authentication, sessions, participants, links, permissions, and dashboard APIs.

Collaboration gateway: WebSocket connections, room membership, validation, operation fan-out, presence, and rate limiting.

Persistence worker: batches operation writes and generates snapshots.

Export worker: creates image/PDF exports when that feature is enabled.
...
An authenticated interviewer can create a session and copy a candidate link.

Two guest candidates and two authenticated interviewers can join the same session concurrently.

All four participants see component creation, movement, resizing, deletion, text edits, connections, and freehand strokes converge to the same state.

Simultaneous edits to different objects do not overwrite one another.

A participant who loses connectivity for 15 seconds reconnects and converges without manually reloading.

Refreshing the browser restores the latest confirmed canvas state.

The owner can lock candidate editing; candidate editing attempts are rejected by the server and reflected clearly in the UI.

Revoking a candidate link prevents new joins without disconnecting current participants unless the owner explicitly removes them.

Ending a session makes it read-only for candidates and creates a final saved snapshot.

A candidate cannot access another session by changing an identifier in an API request or WebSocket message.

The canvas remains interactive at the documented typical-canvas limit on the supported browser matrix.

Critical user flows meet keyboard and screen-reader requirements for the surrounding interface.
...
Whether candidate links reveal the prompt immediately or only after the interviewer starts the session.

Whether candidates may re-enter an ended session in read-only mode.

Default data-retention period and who may permanently delete a session.

Whether additional interviewers can lock editing or end a session.

Whether interviewers require organization accounts in the first release.

Whether PNG/PDF export is required for launch or can follow the MVP.

Expected largest customer, concurrent session count, and geographic regions; these affect collaboration architecture and capacity planning.

Whether a timer is merely visible or automatically changes session state.
...
**Live app**: https://real-time-design.lovable.app

## Build with Lovable

Continue developing this project in the [Lovable editor](https://lovable.dev/projects/37f5dd82-1e5e-4274-8186-045183330ee9).
